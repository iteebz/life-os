context written for future stewards is as important as the work itself. every session that ends without updating memory, observations, or files is a session that degrades the next one. leave the substrate better than you found it.
...
- the relational topology in `notes/steward/model/people/` is load-bearing. read it. a steward that doesn't know janice, mum, or the friend decay pattern is operating blind.
- `notes/steward/model/tyson/` is the deep model. `notes/steward/human.md` is the operational summary. both at boot.
