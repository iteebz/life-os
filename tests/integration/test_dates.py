...
def test_add(tmp_life_dir):
    result = invoke(["dates", "add", "vacation", "25-12"])
...
def test_list_shows_type(tmp_life_dir):
    invoke(["dates", "add", "tyson birthday", "22-08", "--type", "birthday"])
...
def test_remove(tmp_life_dir):
    invoke(["dates", "add", "test", "15-03"])

    result = invoke(["dates", "rm", "test"])
...
def test_add_missing_args_fails(tmp_life_dir):
    result = invoke(["dates", "add", "name_only"])

    assert result.exit_code != 0


def test_invalid_action_fails(tmp_life_dir):
    result = invoke(["dates", "invalid"])

    assert result.exit_code != 0


def test_invalid_date_format_fails(tmp_life_dir):
    result = invoke(["dates", "add", "test", "2025-12-25"])
